...
const products = [
  { id: 'malabar-masala', name: 'Malabar Curry Masala', stock: 32, price: 14.99 },
  { id: 'black-pepper', name: 'Black Peppercorns', stock: 120, price: 9.5 },
  { id: 'cardamom-tea', name: 'Cardamom Chai Blend', stock: 18, price: 13.75 },
]

const orders = [
  { id: 'ORD-101', customer: 'Amala', total: 98.5, status: 'Processing' },
  { id: 'ORD-102', customer: 'Ravi', total: 46.25, status: 'Shipped' },
  { id: 'ORD-103', customer: 'Priya', total: 67.75, status: 'Delivered' },
]
        ...
        <div className="space-y-8">
          <div className="grid gap-6 md:grid-cols-3">
            <div className="rounded-[2rem] border border-[#22c622]/10 bg-[#22c622]/10 p-6 shadow-lg">
              ...
              <p className="mt-4 text-3xl font-semibold text-[#1E3A1A]">₹23.4K</p>
            </div>
            <div className="rounded-[2rem] border border-[#22c622]/10 bg-white p-6 shadow-lg">
              ...
              <p className="mt-4 text-3xl font-semibold text-[#1E3A1A]">134</p>
            </div>
            <div className="rounded-[2rem] border border-[#22c622]/10 bg-white p-6 shadow-lg">
              <p className="text-sm uppercase tracking-[0.25em] text-[#22c622]/80">Products</p>
              <p className="mt-4 text-3xl font-semibold text-[#1E3A1A]">28</p>
            </div>
          </div>
          ...
          {activeTab === 'dashboard' && (
            <div className="rounded-[2rem] border border-[#22c622]/10 bg-white p-8 shadow-xl">
              ...
              <p className="mt-4 text-sm leading-7 text-[#475569]">Metrics are updated in real time. Review products, orders, and stock levels to keep the storefront running smoothly.</p>
              <div className="mt-8 grid gap-6 md:grid-cols-3">
                <div className="rounded-3xl bg-[#FACC15]/10 p-6">
                  ...
                  <p className="mt-4 text-xl font-semibold text-[#1E3A1A]">4 items</p>
                </div>
                <div className="rounded-3xl bg-[#FACC15]/10 p-6">
                  <p className="text-sm uppercase tracking-[0.25em] text-[#22c622]/80">Pending orders</p>
                  <p className="mt-4 text-xl font-semibold text-[#1E3A1A]">12</p>
                </div>
                <div className="rounded-3xl bg-[#FACC15]/10 p-6">
                  <p className="text-sm uppercase tracking-[0.25em] text-[#22c622]/80">New customers</p>
                  <p className="mt-4 text-xl font-semibold text-[#1E3A1A]">34</p>
                </div>
              </div>
            </div>
          )}
                ...
                <table className="min-w-full divide-y divide-[#FACC15]/30 text-left text-sm text-[#1E3A1A]">
                  ...
                  <tbody className="divide-y divide-[#FACC15]/30 bg-white">
                    {products.map((product) => (
                      <tr key={product.id} className="hover:bg-[#22c622]/5 transition duration-200">
                        <td className="px-6 py-5 font-semibold text-[#1E3A1A]">{product.name}</td>
                        <td className="px-6 py-5">{product.stock}</td>
                        <td className="px-6 py-5">₹{product.price.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="rounded-[2rem] border border-[#22c622]/10 bg-white p-8 shadow-xl">
              ...
              <div className="mt-8 overflow-hidden rounded-[1.75rem] border border-[#FACC15]/20">
                <table className="min-w-full divide-y divide-[#FACC15]/30 text-left text-sm text-[#1E3A1A]">
                  <thead className="bg-[#FACC15]/10">
                    <tr>
                      <th className="px-6 py-4 font-semibold uppercase tracking-[0.18em]">Order</th>
                  ...
                  <tbody className="divide-y divide-[#FACC15]/30 bg-white">
                    {orders.map((order) => (
                      <tr key={order.id} className="hover:bg-[#22c622]/5 transition duration-200">
                        <td className="px-6 py-5 font-semibold text-[#1E3A1A]">{order.id}</td>
                        <td className="px-6 py-5">{order.customer}</td>
                        <td className="px-6 py-5">₹{order.total.toFixed(2)}</td>
                        <td className="px-6 py-5 text-[#475569]">{order.status}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
