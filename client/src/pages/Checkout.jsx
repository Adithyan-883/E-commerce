...
const checkoutSchema = z.object({
  ...
  postalCode: z.string().regex(/^\d{6}$/, 'Please enter a valid 6-digit Indian postal code'),
...
const Checkout = () => {
  ...
  if (itemsToCheckout.length === 0) {
    return (
      <section className="mx-auto flex max-w-7xl flex-col items-center justify-center px-4 py-24 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-semibold text-[#1E3A1A]">Your cart is empty</h1>
  ...
  const fieldClass = (hasError) => `mt-3 w-full rounded-3xl border bg-[#FACC15]/10 px-4 py-4 text-sm text-[#1E3A1A] outline-none transition duration-300 ${hasError ? 'border-red-500' : 'border-[#22c622]/20 focus:border-[#22c622]'}`
      ...
      <div className="grid gap-8 lg:grid-cols-[1.4fr,_0.9fr]">
        <form id="checkout-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6 rounded-[2rem] border border-[#22c622]/10 bg-white p-8 shadow-xl sm:p-10">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <label htmlFor="fullName" className="block">
              ...
              <input id="fullName" {...register('fullName')} className={fieldClass(errors.fullName)} />
          ...
          <label htmlFor="phone" className="block">
            ...
            <input id="phone" type="tel" {...register('phone')} className={fieldClass(errors.phone)} placeholder="e.g. 9876543210" />
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            <label htmlFor="postalCode" className="block">
              ...
              <input id="postalCode" {...register('postalCode')} className={fieldClass(errors.postalCode)} />
          ...
          <label htmlFor="country" className="block">
            ...
            <input id="country" {...register('country')} className={fieldClass(errors.country)} />
        ...
        <aside className="space-y-6">
          <div className="rounded-[2rem] border border-[#22c622]/10 bg-[#FACC15]/10 p-8 shadow-xl">
            ...
            <div className="mt-6 space-y-4">
              {itemsToCheckout.map((item) => (
                <div key={item.cartKey || `${item.id}:${item.packLabel || 'standard'}`} className="flex items-center gap-4 rounded-3xl bg-white p-4 shadow-sm">
                  <img src={item.image || item.images?.[0]} alt={item.title} className="h-16 w-16 rounded-3xl object-cover" loading="lazy" />
                  <div>
                    <p className="text-sm font-semibold text-[#1E3A1A]">{item.title}</p>
                    {item.packLabel && <p className="text-xs text-[#475569]">{item.packLabel}</p>}
                    <p className="text-sm text-[#475569]">Qty: {item.quantity} x Rs. {item.price.toFixed(2)}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-8 space-y-3 text-sm text-[#475569]">
              ...
              {discount > 0 && <div className="flex justify-between text-[#22c622]"><span>Discount (10% OFF)</span><span>-Rs. {discount.toFixed(2)}</span></div>}
              <div className="flex justify-between text-base font-semibold text-[#1E3A1A]"><span>Total</span><span>Rs. {total.toFixed(2)}</span></div>
            </div>
          </div>
          <div className="rounded-[2rem] border border-[#22c622]/10 bg-white p-8 shadow-xl">
            ...
            <p className="mt-4 text-sm leading-7 text-[#475569]">Cash on Delivery is available while secure online payments are being integrated.</p>
            <div className="mt-6 rounded-3xl border border-[#22c622]/20 bg-[#FACC15]/10 p-5 text-sm text-[#1E3A1A]">The server confirms current stock and final price when your order is submitted.</div>
          </div>
